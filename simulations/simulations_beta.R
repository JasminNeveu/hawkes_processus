# But: étudier les variations du paramètre beta en jouant sur les paramètres
# Remarque: même code que dans "simulations_alpha.R"
...
hawkes_0 <- gen_process(alpha=alpha_0, beta=b0, mu_h=mu_hawkes)
hawkes_1 <- gen_process(alpha=alpha_0, beta=b1, mu_h=mu_hawkes)
hawkes_2 <- gen_process(alpha=alpha_0, beta=b2, mu_h=mu_hawkes)
hawkes_3 <- gen_process(alpha=alpha_0, beta=b3, mu_h=mu_hawkes)

max1 = length(hawkes_1)
max2 = length(hawkes_2)

# Création des 4 graphiques
p1 <- ggplot() + 
  geom_step(aes(x=hawkes_0, y=0:(length(hawkes_0)-1)))+
  labs(title=paste0("Processus de Hawkes, beta = ", b0), x="t", y="N(t)") + 
  ylim(c(0, max1)) +
  theme_bw()

p2 <- ggplot() + 
  geom_step(aes(x=hawkes_1, y=0:(length(hawkes_1)-1))) +
  labs(title = paste0("Processus de Hawkes, beta = ", b1), x="t", y="N(t)") + 
  ylim(c(0, max1)) +
  theme_bw()

p3 <- ggplot() + 
  geom_step(aes(x=hawkes_2, y=0:(length(hawkes_2)-1))) +
  labs(title = paste0("Processus de Hawkes, beta = ", b2), x="t", y="N(t)") + 
  ylim(c(0, max2)) +
  theme_bw()

p4 <- ggplot() + 
  geom_step(aes(x=hawkes_3, y=0:(length(hawkes_3)-1))) +
  labs(title = paste0("Processus de Hawkes, beta = ", b3), x="t", y="N(t)") +
  ylim(c(0, max2)) +
  theme_bw()
